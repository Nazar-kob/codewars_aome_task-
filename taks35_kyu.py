# A binary gap within a positive number num is any sequence
...
# Write function gap(num) that,  given a positive num,  returns the
# length of its longest binary gap.
# The function should return 0 if num doesn't contain a binary gap.


def gap(num):
    bin_len = str(bin(num))[2:] +"_"
    return max([len(i) for i in set(bin_len.split('1')) if i.count('0') == len(i)])